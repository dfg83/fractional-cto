...
2. Map events to the chosen angle's key beats
3. Group beats into logical sections (typically 4-8 sections)
4. For each beat, determine treatment: `quote`, `summarize`, `montage`, or `skip`
5. Select the strongest quotes — clean obvious typos, merge consecutive fragments
6. Estimate word count per section (total should match the angle's target)
7. Identify open questions where the author's input would improve the outline
